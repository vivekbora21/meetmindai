...
def check_and_setup_db():
    ...
    has_docker = shutil.which("docker") is not None or shutil.which("docker-compose") is not None
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt == max_retries:
                print("Could not connect to the database. Please ensure your PostgreSQL service is running and configured correctly.")
                raise e
            
            print(f"Database connection failed: {e}")
            
            if skip_docker or not has_docker:
                print("Skipping automatic Docker Compose startup (Docker not found or SKIP_DOCKER is set).")
            ...
            if not docker_started:
                print("Warning: Could not start Docker services automatically. Please ensure they are running.")
