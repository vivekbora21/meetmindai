...
from sqlalchemy import (
    ...
    Table, Text, Boolean, JSON, Index, func
)
...
class KnowledgeGraphNode(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    organization_id = Column(String(36), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False)
    entity_type = Column(String(50), nullable=False) # Person, Project, Technology, Meeting, Repository
    name = Column(String(255), nullable=False)
    description = Column(Text, nullable=True)
    meta_data = Column(JSON, nullable=True)

    __table_args__ = (
        Index("idx_org_entity_name", "organization_id", "entity_type", "name", unique=True),
    )
...
class KnowledgeGraphEdge(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    organization_id = Column(String(36), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False)
    source_node_id = Column(String(36), ForeignKey("knowledge_graph_nodes.id", ondelete="CASCADE"), nullable=False)
    target_node_id = Column(String(36), ForeignKey("knowledge_graph_nodes.id", ondelete="CASCADE"), nullable=False)
    relationship_type = Column(String(100), nullable=False) # e.g., "MEMBER_OF", "USES", "DISCUSSED_IN"
...
class AgentProfile(Base):
    ...
    auto_join_rules = Column(JSON, nullable=True) # {"internal_only": true, "max_duration_minutes": 120}
    created_at = Column(DateTime, server_default=func.now())
...
class ScheduledMeeting(Base):
    ...
    status = Column(String(50), default="Scheduled") # 'Scheduled', 'Joined', 'Completed', 'Failed'
    created_at = Column(DateTime, server_default=func.now())
...
class AgentLiveSession(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    meeting_id = Column(String(36), ForeignKey("meetings.id", ondelete="SET NULL"), nullable=True)
    scheduled_meeting_id = Column(String(36), ForeignKey("scheduled_meetings.id", ondelete="SET NULL"), nullable=True)
    status = Column(String(50), nullable=False, default="Connecting") # 'Connecting', 'Live', 'Completed', 'Error'
