...
class BackgroundSchedulerService:
        ...
        for acc in expiring_accounts:
            try:
                ...
                if acc.provider == Provider.GOOGLE:
                    new_tokens = await self.google_oauth.refresh_tokens(acc.refresh_token)
                    ...
                    new_tokens = await self.microsoft_oauth.refresh_tokens(acc.refresh_token)
                    from app.utils.encryption import encrypt_value
                    acc.access_token = encrypt_value(new_tokens["access_token"])
                    if new_tokens.get("refresh_token"):
                        acc.refresh_token = encrypt_value(new_tokens["refresh_token"])
    ...
    async def sync_calendars(self):
        ...
        for acc in accounts:
            try:
                ...
                logger.error(f"Scheduler | Calendar sync failed for connected account {acc.id}: {e}")
                acc.sync_errors = f"Sync failed: {str(e)}"
                self.db.commit()
    ...
    async def retry_failed_syncs(self):
        """Retries calendar synchronization for connected accounts that have sync errors."""
        failed_accounts = self.db.query(ConnectedAccount).filter(
            ConnectedAccount.connection_status == "Connected",
            ConnectedAccount.sync_errors.isnot(None)
        ).all()

        logger.info(f"Scheduler | Retrying {len(failed_accounts)} accounts with sync errors.")
        for acc in failed_accounts:
            try:
                ...
                logger.warning(f"Scheduler | Retry failed sync again for connected account {acc.id}: {e}")
