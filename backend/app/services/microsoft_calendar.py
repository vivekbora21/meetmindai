        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                ...
                if cals_resp.status_code == 200:
                    ...
                    logger.info(f"Found {len(calendars)} personal calendars for user {user_id}")
                    for cal in calendars:
                        ...
                        try:
                            view_resp = await client.get(view_url, headers=headers, timeout=10.0)
                            if view_resp.status_code == 200:
                                evs = view_resp.json().get("value", [])
                                logger.info(f"Fetched {len(evs)} events from calendar '{cal_name}'")
                                all_graph_events.extend(evs)
                            else:
                                logger.warning(f"Failed to fetch calendarView for calendar '{cal_name}': {view_resp.status_code}")
                        except Exception as e:
                            logger.error(f"Error fetching calendarView for calendar '{cal_name}': {e}")
                else:
                    logger.warning(f"Failed to list user calendars: {cals_resp.status_code}, falling back to primary calendarView")
                    fallback_url = f"https://graph.microsoft.com/v1.0/me/calendarView?startDateTime={start_str}&endDateTime={end_str}"
                    fb_resp = await client.get(fallback_url, headers=headers, timeout=10.0)
                    if fb_resp.status_code == 200:
                        all_graph_events.extend(fb_resp.json().get("value", []))
            except Exception as e:
                logger.error(f"Error checking user calendars: {e}, falling back to primary calendarView")
                fallback_url = f"https://graph.microsoft.com/v1.0/me/calendarView?startDateTime={start_str}&endDateTime={end_str}"
                try:
                    fb_resp = await client.get(fallback_url, headers=headers, timeout=10.0)
                    if fb_resp.status_code == 200:
            ...
            try:
                ...
                if account.email:
                    ...
                    personal_domains = (
                        ...
                        "@gmail.com", "@yahoo.com", "@icloud.com", "@aol.com",
                ...
                if is_personal:
                    logger.info(f"Skipping Teams groups sync for user {user_id} (not supported for personal accounts: {account.email})")
                else:
                    logger.info(f"Listing joined Teams groups for user {user_id}")
                    teams_url = "https://graph.microsoft.com/v1.0/me/joinedTeams"
                    teams_resp = await client.get(teams_url, headers=headers, timeout=10.0)
                    if teams_resp.status_code == 200:
                        teams = teams_resp.json().get("value", [])
                        logger.info(f"Found {len(teams)} joined Teams/Groups for user {user_id}")
                        for team in teams:
                            team_id = team.get("id")
                            ...
                            try:
                                t_resp = await client.get(team_view_url, headers=headers, timeout=10.0)
                                if t_resp.status_code == 200:
                                    t_evs = t_resp.json().get("value", [])
                                    logger.info(f"Fetched {len(t_evs)} events from Teams Group '{team_name}'")
                                    all_graph_events.extend(t_evs)
                                else:
                                    logger.warning(f"Failed to fetch calendarView for Teams Group '{team_name}' (Status {t_resp.status_code})")
                            except Exception as e:
                                logger.error(f"Error fetching calendarView for Teams Group '{team_name}': {e}")
                    else:
                        error_msg = ""
                        try:
                        ...
                        log_msg = f"Failed to list joined Teams: {teams_resp.status_code}"
                        if error_msg:
                            log_msg += f" - {error_msg}"
            ...
            try:
                ...
                logger.info(f"Successfully synced and committed {len(synced_events)} total events for user {user_id}.")
