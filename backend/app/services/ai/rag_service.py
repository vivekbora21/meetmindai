...
def get_embedding_model():
    ...
    if _embedding_model is None:
        try:
            ...
            logger.error(
                f"Failed to load sentence transformer model BAAI/bge-small-en-v1.5: {e}"
            )
...
def get_reranker_model():
    ...
    if _reranker_model is None:
        try:
            ...
            logger.error(
                f"Failed to load cross encoder model BAAI/bge-reranker-base: {e}"
            )
...
class RAGService:
    ...
    def chunk_transcript(cls, segments: List[Transcript]) -> List[Dict[str, Any]]:
        ...
        while start_idx < n:
            ...
            while end_idx < n:
                ...
                formatted_seg = (
                    f"[{cls.format_time(seg.start_time)}] {speaker_name}: {seg.text}\n"
                )
            ...
            speakers = list(
                dict.fromkeys(item[0].speaker_tag for item in current_segments)
            )
    ...
    def index_meeting(cls, db: Session, meeting_id: str) -> bool:
        ...
        try:
            ...
            if not segments:
                logger.warning(
                    f"RAGService | No transcripts found for meeting: {meeting_id}"
                )
                ...
                if model is not None:
                    try:
                        embedding = model.encode(
                            chunk_text, convert_to_numpy=True
                        ).tolist()
                    except Exception as e:
            ...
            logger.error(
                f"RAGService | Failed to index meeting {meeting_id}: {e}", exc_info=True
            )
    ...
    def rrf_merge(
        cls,
        vector_list: List[MeetingChunk],
        keyword_list: List[MeetingChunk],
        k: int = 60,
    ) -> List[MeetingChunk]:
        """
    ...
    def search_chunks(
        ...
        if filters.get("platform"):
            base_query = base_query.filter(
                Meeting.platform.ilike(f"%{filters['platform']}%")
            )
        ...
        if query_embedding is not None:
            try:
                ...
                vector_results = (
                    base_query.order_by(distance_expr).limit(limit * 2).all()
                )
            except Exception as e:
        ...
        if words:
            ...
            keyword_results = base_query.filter(or_(*kw_filters)).limit(limit * 2).all()
            ...
            chunk_items.append(
                {
                    ...
                    "meeting_date": (
                        chunk.meeting.meeting_date.strftime("%Y-%m-%d")
                        if chunk.meeting.meeting_date
                        else "Unknown Date"
                    ),
    ...
    def get_suggested_questions(cls, chunks: List[Dict[str, Any]]) -> List[str]:
        ...
        if "action" in joined_text or "task" in joined_text or "todo" in joined_text:
            suggestions.append(
                "What action items or next steps were assigned and to whom?"
            )
        if (
            "deadline" in joined_text
            or "due" in joined_text
            or "date" in joined_text
            or "by when" in joined_text
        ):
            suggestions.append(
                "What are the upcoming project deadlines or delivery schedules?"
            )
        if (
            "decision" in joined_text
            or "agreed" in joined_text
            or "decided" in joined_text
        ):
            suggestions.append(
                "What major decisions were agreed upon during the meeting?"
            )
        if (
            "risk" in joined_text
            or "issue" in joined_text
            or "block" in joined_text
            or "concern" in joined_text
        ):
            suggestions.append(
                "What risks or blockers were discussed, and how will they be mitigated?"
            )
    ...
    def chat_answer(
        ...
        sources = cls.search_chunks(
            db, current_user, question, filters=filters, limit=6
        )
        ...
        try:
            ...
            answer = (
                "I encountered an error while retrieving your answer. Please try again."
            )
        ...
        if sources:
            ...
            confidence = (
                float(1.0 / (1.0 + math.exp(-avg_score)))
                if provider != "ollama"
                else 0.8
            )
    ...
    def chat_answer_stream(
        ...
        logger.info(
            f"RAGService | Invoking streaming LLM: {model} ({provider}) for RAG Chat"
        )
