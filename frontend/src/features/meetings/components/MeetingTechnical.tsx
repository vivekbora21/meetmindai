    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {detail.recording_url && (
        <div className="p-4 rounded-2xl bg-white border border-slate-200 flex flex-col gap-2 shadow-sm md:col-span-2">
          <h4 className="text-xs font-bold text-[#0f172a] flex items-center gap-1.5 font-outfit">
            <FileAudio className="w-3.5 h-3.5 text-[#0f766e]" /> Uploaded File Details
          </h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs text-slate-500 font-medium">
            ...
            <div>
              ...
              <a href={audioSrc} target="_blank" rel="noreferrer" className="text-[#0f766e] hover:underline break-all">
                {detail.recording_url}
              </a>
            </div>
          </div>
      ...
      <div className="p-4 rounded-2xl bg-white border border-slate-200 flex flex-col gap-2 shadow-sm">
        <h4 className="text-xs font-bold text-[#0f172a] flex items-center gap-1.5 font-outfit">APIs & Tables</h4>
        <ul className="text-xs text-slate-500 flex flex-col gap-1">
          {detail.technical_context?.apis?.map((api: string, idx: number) => (
            <li key={idx} className="font-mono text-[10px] bg-slate-50 border border-slate-200 px-1 py-0.5 rounded w-fit text-slate-600">
              {api}
            </li>
          )) || <li className="text-slate-400">None</li>}
          {detail.technical_context?.database_tables?.map((table: string, idx: number) => (
            <li key={idx} className="text-[#0f172a] font-medium">{table}</li>
          )) || <li className="text-slate-400">None</li>}
        </ul>
      </div>
    </div>
