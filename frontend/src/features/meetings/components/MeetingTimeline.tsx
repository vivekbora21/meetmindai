      ...
      {detail.agenda_items ? detail.agenda_items.map((item: any, idx: number) => (
        <div key={idx} className="p-4 rounded-2xl bg-white border border-slate-200 flex flex-col gap-2 shadow-sm">
          <div className="flex justify-between items-center">
            <span className="text-sm font-bold text-[#0f172a] font-outfit">{item.topic}</span>
            <span className="text-[10px] text-[#0f766e] font-bold bg-teal-50 border border-teal-100 px-2 py-1 rounded-lg flex items-center gap-1 shadow-sm">
